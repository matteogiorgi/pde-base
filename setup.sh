...
NC='\033[0m'
# ---
if [[ -d "${HOME}/.confrestore" ]]; then
    printf "\n${RED}%s${NC}"   "════════ Warning: pde-conf already set ════════"
    printf "\n${RED}%s${NC}\n" "Remove ~/.confrestore and run this script again"
    exit 1
fi
# ---
...
function store-conf () {
    ...
    RESTORE="${HOME}/.confrestore" && command mkdir -p "${RESTORE}"
...
command sudo apt-get install -qq -y git xclip trash-cli bash bash-completion python3 vim-gtk3 tmux \
      wamerican fd-find fzy fonts-liberation input-remapper diodon || error-echo "installing from apt"
# ---
store-conf
